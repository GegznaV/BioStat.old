# ------------------------------------------------------------------------------
#' @export
describe_data.vector <- function(x,
                            descript,
...

{
    ...
    if (exclude.missing && n == 0)
        ...
        return(class_stat_summary(list()))
    ...
    isdot <- Hmisc::testDateTime(x, 'either') # is date or time var
    isdat <- Hmisc::testDateTime(x, 'both')   # is date and time combo var
    ...
    timeUsed <- isdat && Hmisc::testDateTime(x.unique, 'timeVaries')
    ...
    kalba <- list()
    kalba$n                   <- "n"
    kalba$missing             <- "missing"
    kalba$imputed             <- "imputed"
    kalba$missing_month       <- "missing month"
    kalba$missing_day         <- "missing day"
    kalba$"missing month,day" <- "missing month,day"
    kalba$distinct            <- "distinct"
    kalba$Info            <- "Info"
    kalba$sum            <- "sum"


    lab <- c("n", "missing") # [!!!] language
    # lab <- c(kalba$n,  kalba$missing ) # [!!!] language
    ...
    if (isnum) {
        if (isdot) {
            ...
            fval <- Hmisc::formatDateTime(dd, atx, !timeUsed)
        ...
        if (!weighted) {
            ...
            counts <-
                c(counts,
                  if (isdot) {
                      Hmisc::formatDateTime(gmd, atx, !timeUsed)
                  } else {
                      format(gmd, ...)
                  })
    ...
    if (n.unique >= 10 & isnum) {
        ...
        fval <- if (isdot) {
            Hmisc::formatDateTime(q, atx, !timeUsed)
        } else {
            format(q, ...)
        }
    ...
    if (!x.binary) {
        if (inherits(x, 'mChoice')) {
            ...
            if (isnum) {
                if (n.unique >= 100 || min(diff(sort(unique(xnum)))) < diff(range(xnum)) / 500) {
